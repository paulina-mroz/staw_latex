...
def cardCaptainTex (card, size):
    ...
    tex = tex + tikzResizedTextNode("title_style", panel_outer_x1 + 1.3*panel_thick + title_offset, panel_outer_y2 - title_height/2, 0.35, 1.0, title_height - 0.1, card["name"].upper()) + "\n"
    ...
    if card["unique"]:
        circle1_r = 0.8*panel_thick
        circle1_x = panel_outer_x1+0.9*circle1_r
        circle1_y = panel_outer_y2 - title_height/2
        tex = tex + r"\begin{scope}" + "\n"
        tex = tex + tikzCircle("draw=none, fill=panel_fill", circle1_x, circle1_y, circle1_r) + "\n"
        tex = tex + tikzCircle("draw=none, fill=title_fill", circle1_x, circle1_y, circle1_r-0.05) + "\n"
        tex = tex + r"\end{scope}" + "\n"
        tex = tex + r"\begin{scope}" + "\n"
        tex = tex + tikzCircle("clip", circle1_x, circle1_y, circle1_r-0.05) + "\n"
        tex = tex + tikzTextNode("text_icon_style", circle1_x, circle1_y, tikzExternalGraphics(1.4*circle1_r, "../pics_vector/unique.pdf" )) + "\n"
        tex = tex + r"\end{scope}" + "\n"
